...
fn cuda_library_search_paths() -> Vec<PathBuf> {
    ...
    let mut push_if_exists = |candidate: PathBuf| {
        if candidate.exists() && !paths.contains(&candidate) {
            paths.push(candidate);
        }
    };

    push_if_exists(PathBuf::from("/lib/x86_64-linux-gnu"));

    if let Some(base) = env::var_os("CUDA_HOME").or_else(|| env::var_os("CUDA_PATH")) {
        let base_path = PathBuf::from(base);
        push_if_exists(base_path.join("targets/x86_64-linux/lib"));
        push_if_exists(base_path.join("lib64"));
    }

    push_if_exists(PathBuf::from("/usr/local/cuda/targets/x86_64-linux/lib"));
    push_if_exists(PathBuf::from("/usr/local/cuda/lib64"));
